    ...
    public void generate() throws IOException {
        ...
        for (Element e : typeElement.getEnclosedElements()) {
            if (e.getKind() == ElementKind.FIELD && !e.getModifiers().contains(Modifier.STATIC)) {
                ...
                allFields.add(e);
                cn.kunter.dynamic.annotations.TableId tableId = e.getAnnotation(cn.kunter.dynamic.annotations.TableId.class);
                if (tableId != null) {
                    pkElement = e;
                    autoIncrement = tableId.autoIncrement();
                    pkColumnName = tableId.value().isEmpty() ? cn.kunter.dynamic.processor.utils.StringUtils.camelToSnake(e.getSimpleName().toString()) : tableId.value();
                }
            }
        }
        ...
        TypeSpec.Builder mapperInterfaceBuilder = TypeSpec.interfaceBuilder(mapperClassName)
                ...
                .addAnnotation(AnnotationSpec.builder(mapperAnnotation).build())
                .addSuperinterface(commonSelect)
                .addSuperinterface(commonUpdate)
                .addSuperinterface(commonDelete)
        ...
        if (pkElement != null) {
            ...
            com.squareup.javapoet.MethodSpec selectByPk = com.squareup.javapoet.MethodSpec.methodBuilder("selectByPrimaryKey")
                    ...
                    .addStatement("return selectOne($T.select($T.$L.allColumns()).from($T.$L).where($T.$L, $T.isEqualTo($L_)).build().render($T.MYBATIS3))", 
                        sqlBuilderClass, supportClass, tableFieldName, 
                        supportClass, tableFieldName, 
                        supportClass, pkName, sqlBuilderClass, pkName, renderingStrategies)
                    .build();
            mapperInterfaceBuilder.addMethod(selectByPk);

            // deleteByPrimaryKey
            com.squareup.javapoet.MethodSpec deleteByPk = com.squareup.javapoet.MethodSpec.methodBuilder("deleteByPrimaryKey")
                    .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                    .returns(int.class)
                    .addParameter(pkType, pkName + "_")
                    .addStatement("return delete($T.deleteFrom($T.$L).where($T.$L, $T.isEqualTo($L_)).build().render($T.MYBATIS3))", 
                        sqlBuilderClass, supportClass, tableFieldName, 
                        supportClass, pkName, sqlBuilderClass, pkName, renderingStrategies)
                    .build();
            ...
            // updateByPrimaryKey
            com.squareup.javapoet.MethodSpec.Builder updateByPkBuilder = com.squareup.javapoet.MethodSpec.methodBuilder("updateByPrimaryKey")
                    .addModifiers(Modifier.PUBLIC, Modifier.DEFAULT)
                    .returns(int.class)
            ...
            if (autoIncrement) {
                ...
                ClassName insertStatementProvider = ClassName.get("org.mybatis.dynamic.sql.insert.render", "InsertStatementProvider");
                ClassName multiRowInsertStatementProvider = ClassName.get("org.mybatis.dynamic.sql.insert.render", "MultiRowInsertStatementProvider");

                com.squareup.javapoet.MethodSpec insert = com.squareup.javapoet.MethodSpec.methodBuilder("insert")
                        .addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
                        .returns(int.class)
                        .addAnnotation(AnnotationSpec.builder(insertProvider).addMember("type", "$T.class", sqlProviderAdapter).addMember("method", "$S", "insert").build())
                        .addAnnotation(AnnotationSpec.builder(options).addMember("useGeneratedKeys", "true").addMember("keyProperty", "$S", "row." + pkName).build())
                        .addParameter(ParameterizedTypeName.get(insertStatementProvider, entityClass), "insertStatement")
                        .build();
                mapperInterfaceBuilder.addMethod(insert);

                com.squareup.javapoet.MethodSpec insertMultiple = com.squareup.javapoet.MethodSpec.methodBuilder("insertMultiple")
                        .addModifiers(Modifier.PUBLIC, Modifier.ABSTRACT)
                        .returns(int.class)
                        .addAnnotation(AnnotationSpec.builder(insertProvider).addMember("type", "$T.class", sqlProviderAdapter).addMember("method", "$S", "insertMultiple").build())
                        .addAnnotation(AnnotationSpec.builder(options).addMember("useGeneratedKeys", "true").addMember("keyProperty", "$S", "records." + pkName).build())
                        .addParameter(ParameterizedTypeName.get(multiRowInsertStatementProvider, entityClass), "multipleInsertStatement")
                        .build();
                mapperInterfaceBuilder.addMethod(insertMultiple);
            }
        }

        JavaFile javaFile = JavaFile.builder(packageName, mapperInterfaceBuilder.build())
                .indent("    ")
                .build();
        try {
